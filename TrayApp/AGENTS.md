...
| WS 协议 | `WebSocketServer.cs:118-155` | `ProcessCommand()` — JSON 分发 |
| 插件加载 | `PluginManager.cs:105-148` | `TryLoadPlugin()` — Assembly.LoadFrom |
| 参数转换 | `PluginManager.cs:40-86` | 通过 `Convert.ChangeType` 类型转换 |
| 退出清理 | `App.xaml.cs:137-159` | 优雅关闭 |

## 约定
- 插件 DLL 放在应用基目录的 `plugins/` 子目录
- 插件类：`{插件名}.{插件名}`（命名空间.类名）
- 方法匹配：不区分大小写，按参数数量和类型转换最佳匹配
- 日志：每日滚动文件 `TrayApp_yyyyMMdd.log`
- WS 端口：8761（Windows 用 localhost，Linux 用 0.0.0.0）
